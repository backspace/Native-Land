...
if($mapsRequest) {
    ...
    foreach ($mapsRequestArray as $key => $value) {
        if($value=='languages') {
            $languageResults = return_results('https://native-land.ca/coordinates/indigenousLanguages.json',$nameRequest,$langRequest);
            $result = array_merge($result,$languageResults);
        }
        if($value=='territories') {
            // Get territories
            $territoryResults = return_results('https://native-land.ca/coordinates/indigenousTerritories.json',$nameRequest,$langRequest);
            $result = array_merge($result,$territoryResults);
        }
        if($value=='treaties') {
            // Get treaties
            $treatyResults = return_results('https://native-land.ca/coordinates/indigenousTreaties.json',$nameRequest,$langRequest);
...
function return_results($fileURL,$nameRequest,$langRequest) {
    $thisFile = file_get_contents_curl($fileURL);
                ...
                foreach ($arr as $obj) {
                    if($obj->Slug==$value2) {
                        ...
                        if($langRequest=='fr') {
                            ...
                            if($fileURL=='https://native-land.ca/coordinates/indigenousTreaties.json') {
